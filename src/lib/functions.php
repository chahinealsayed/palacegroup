...
function createUrl($path, $args = [], $basePath = null)
{
    if (!$basePath) {
        $basePath = tier()->path;
    }
    ...
    Url::$urlManager->setBaseUrl($basePath);
    $url = Url::to($path, true);
    if (tier()->path == "/") {
        return $url;
    }
    return str_replace(tier()->path, "", $url);
}
...
function sideMenu()
{
    return [
        [
            ...
            'options' => ['class' => Yii::$app->request->getUrl() === '/default/index' ? 'active' : ''],

        ],
        [
            'label' => 'About Us',
            'url' =>  Url::to(['default/content', 'id' => 'cms.contents.default.about']),
            'options' => ['class' => Yii::$app->request->getUrl() === '/About' ? 'active' : ''],

        ],
        [
            'label' => 'Services',
            'url' =>  Url::to(['default/contents', 'type' => 'services']),
            'options' => ['class' => Yii::$app->request->getUrl() === '/Services' ? 'active' : ''],
        ],
        [
            'label' => 'Divisions',
            'url' =>  Url::to(['default/contents', 'type' => 'divisions']),
            'options' => ['class' => Yii::$app->request->getUrl() === '/Divisions' ? 'active' : ''],
        ],

        [
            'label' => 'Projects',
            'url' =>  Url::to(['default/contents', 'type' => 'projects']),
            'options' => ['class' => Yii::$app->request->getUrl() === '/Projects' ? 'active' : ''],
        ],
        [
            'label' => 'Recruitment',
            'url' =>  Url::to(['default/contents', 'type' => 'recruitment']),
            'options' => ['class' => Yii::$app->request->getUrl() === '/Recruitment' ? 'active' : ''],

        ],
        [
            'label' => 'Contact us',
            'url' =>  Url::to(['default/contents', 'type' => 'contact']),
            'options' => ['class' => Yii::$app->request->getUrl() === '/Contact' ? 'active' : ''],
